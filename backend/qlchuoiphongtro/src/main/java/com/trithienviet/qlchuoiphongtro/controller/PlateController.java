...
public class PlateController {
    ...
    // Thêm ObjectMapper để in JSON đẹp
    ...
    public ResponseEntity<String> receivePlateFromAI(@RequestBody PlatePayload payload) {
        ...
        // In thông tin ảnh base64 (chỉ in độ dài, không in toàn bộ)
        ...
        if ("SUCCESS".equals(payload.getStatus())) {
            ...
            // ==========================================
            // LOGIC NGHIỆP VỤ CỦA BẠN VIẾT Ở ĐÂY:
            // ==========================================
            
            // 1. Lưu vào Database bảng `parking_history`
            // parkingService.saveParkingEntry(payload);
            
            // 2. Check xem biển này có phải xe đăng ký tháng không?
            // boolean isRegistered = parkingService.checkRegisteredPlate(plate);
            
            // 3. Nếu hợp lệ -> Gọi API kích hoạt mở Barie
            // if (isRegistered) {
            //     openBarrier();
            // }
            
            // Phát sóng (Broadcast) dữ liệu này xuống Frontend React
            ...
            // Lưu log cảnh báo hoặc báo bảo vệ kiểm tra thủ công
        ...
        return ResponseEntity.ok("Java Backend đã nhận thành công!");
    }
}