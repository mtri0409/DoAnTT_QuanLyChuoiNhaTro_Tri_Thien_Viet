...
public class ContractServiceImpl implements ContractService {
    ...
    public ContractDTO createContract(ContractDTO contractRequest) {
        ...
        newContract.setRentPrice(room.getPrice()); // tự động lấy từ giá phòng
        newContract.setDepositAmount(roomDeposit.getAmount()); // tự động lấy từ deposit
        ...
        if (contractRequest.getContractServices() != null && !contractRequest.getContractServices().isEmpty()) {
            for (ContractServiceDTO serviceRequest : contractRequest.getContractServices()) {
                ...
                contractService.setPriceAtSigning(serviceItem.getPrice()); // snapshot giá lúc ký
                contractService.setUnitAtSigning(serviceItem.getUnit()); // snapshot đơn vị lúc ký
    ...
    public void addServices(Long contractId, List<ContractServiceDTO> serviceRequests) {
        ...
        for (ContractServiceDTO serviceRequest : serviceRequests) {
            ...
            if (serviceAlreadyExists) {
                continue; // bỏ qua nếu dịch vụ đã tồn tại trong hợp đồng
            }
            ...
            contractService.setService(serviceItem);
            contractService.setPriceAtSigning(serviceItem.getPrice()); // snapshot giá lúc thêm
            contractService.setUnitAtSigning(serviceItem.getUnit()); // snapshot đơn vị lúc thêm
            contractServiceRepo.save(contractService);
        }
    }
