...
import torch
import base64  # Thêm dòng này vào đầu file với các import khác
...
FRAMES_TO_COLLECT = 5
JAVA_API_URL = "http://localhost:8080/api/ai/receive-plate" # Webhook Backend Java
    ...
    def run(self):
        while not self.stopped:
            ...
            payload = {
                ...
                "plate_image_base64": best_crop_base64  # ✅ THÊM TRƯỜNG NÀY
            }
            ...
            try:
                ...
                # Gửi request và hứng kết quả trả về vào biến response
                response = requests.post(JAVA_API_URL, json=payload, timeout=2)
                
                # In chi tiết dựa trên HTTP Status Code
        ...
        while not self.stopped:
            ...
            for crop, class_name in crops_data:
                ...
                try:
                    ...
                    raw_reads.append(raw_text if raw_text.strip() else "[BLANK]")
                    valid_plate = clean_and_validate_plate(raw_text)
                    if valid_plate: valid_candidates.append(valid_plate)
...
# ==================== CÁC API UPLOAD ẢNH TĨNH CŨ ====================
# (Code xử lý đồng hồ nước & upload một tấm ảnh tĩnh vẫn giữ nguyên để web gọi)
