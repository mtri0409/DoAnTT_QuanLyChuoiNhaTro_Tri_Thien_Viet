...
import { FaHome, FaBuilding, FaBed, FaUsers, FaFileInvoiceDollar, FaTimes, FaCog, FaUserShield } from 'react-icons/fa';
...
const NAV_ITEMS = [
  ...
  { title: 'Tài Khoản',     path: '/users',  icon: <FaUserShield /> },
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          transform: window.innerWidth < 768 && !showMobile ? 'translateX(-100%)' : 'translateX(0)',
        }}
      >
        {/* Logo */}
        <div className="d-flex align-items-center gap-2 p-3 border-bottom border-secondary" style={{ minHeight: 72 }}>
          <div
          ...
          <button className="btn btn-link text-white ms-auto p-0 d-md-none" onClick={toggleMobile}>
            <FaTimes />
          </button>
        </div>
        ...
        <nav className="flex-grow-1 p-2 overflow-auto">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: 17, flexShrink: 0, width: 20, textAlign: 'center' }}>
                  {item.icon}
                </span>
                <span
                  style={{
          ...
          >
            ...
            {!isCollapsed && (
              <div style={{ overflow: 'hidden' }}>
                <div className="text-white fw-semibold" style={{ fontSize: 13, whiteSpace: 'nowrap' }}>Tri Admin</div>
                <div className="text-secondary" style={{ fontSize: 11, whiteSpace: 'nowrap' }}>Quản trị viên</div>
              </div>
            )}
          </div>
        </div>
