...
const MOCK_CAMERAS = [
  ...
  {
    id: "CH-03",
    name: "CAMERA BÃI XE TẦNG G (MOCK)",
    streamUrl:
      "https://images.unsplash.com/photo-1573348722427-f1d6819fdf98?q=80&w=600",
    isMock: true,
  },
];
...
const CameraDashboard = () => {
    ...
    stompClient.onConnect = () => {
      ...
      stompClient.subscribe("/topic/plates", (message) => {
        try {
          ...
          // ÁNH XẠ CHÍNH XÁC DỮ LIỆU TỪ JAVA DTO SANG REACT UI
          const standardData = {
            ...newData,
            // Lấy ID từ logId của Java
            track_id: newData.logId || "N/A",

            // Lấy biển số từ licensePlate của Java
            best_plate: newData.licensePlate || "UNKNOWN",

            // Lấy thời gian từ detectedAt của Java
            timestamp: newData.detectedAt || new Date().toISOString(),

            // Lấy độ tin cậy từ confidence của Java (Có thể nhân 100 để ra phần trăm)
            confidence_votes: newData.confidence
              ? `${(newData.confidence * 100).toFixed(0)}%`
              : "N/A",

            // Ép cứng trạng thái là thành công để UI hiện màu xanh
            status: "SUCCESS",
          };
  ...
  const successCount =
    plateHistory?.filter((p) => p?.status === "SUCCESS").length || 0;
  const errorCount =
    plateHistory?.filter((p) => p?.status !== "SUCCESS").length || 0;
    ...
    >
      ...
      <header className="d-flex justify-content-between align-items-center p-3 bg-white border-bottom shadow-sm flex-shrink-0">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            ...
            <small className="text-muted fw-semibold">
              Hệ thống giám sát phương tiện đa kênh
            </small>
          </div>
        </div>
        ...
        <div className="d-flex gap-2">
          <StatusBadge
            label={activeCam.id}
            isOnline={activeCam.isMock ? true : !isStreamOffline}
            icon={<FaVideo />}
          />
          <StatusBadge
            label="Máy chủ Java"
        ...
        <div className="row h-100 g-4">
          ...
          <div className="col-lg-8 d-flex flex-column h-100 gap-3">
            ...
            <div className="card flex-grow-1 bg-dark text-white border-0 shadow overflow-hidden position-relative">
              <div
                className="position-absolute top-0 w-100 p-3 d-flex justify-content-between align-items-center"
                style={{ background: "rgba(0,0,0,0.6)", zIndex: 10 }}
              >
                <div className="d-flex align-items-center gap-2">
                  <span className="badge border border-secondary text-light bg-dark">
                    {activeCam.id}
                  </span>
                ...
                <div className="d-flex align-items-center gap-2">
                  ...
                  {(!isStreamOffline || activeCam.isMock) && (
                    <FaCircle className="text-danger small animate-pulse" />
                  )}
              ...
              <div className="h-100 d-flex align-items-center justify-content-center bg-black">
                {!isStreamOffline || activeCam.isMock ? (
                  <img
                    ...
                    style={{
                      objectFit: activeCam.isMock ? "cover" : "contain",
                    }}
                    onError={() => {
                      if (!activeCam.isMock) setIsStreamOffline(true);
                    }}
                  />
                ) : (
                  <div className="text-center p-5">
                    ...
                    <p className="text-muted small mb-4">
                      Vui lòng kiểm tra lại cổng chạy Python AI Service (Port
                      8000) hoặc liên kết webcam.
                    </p>
                    <button
                      onClick={() => setIsStreamOffline(false)}
              ...
              >
                {totalCount === 0 ? (
                  <div className="h-100 d-flex flex-column align-items-center justify-content-center text-muted">
                    ...
                    <small>
                      Đang nghe tín hiệu xe từ camera hoặc mock dữ liệu...
                    </small>
                  </div>
                ) : (
...
const StatusBadge = ({ label, isOnline, icon }) => {
  ...
  return (
    <div
      className={`border rounded p-2 d-flex align-items-center gap-2 ${badgeClass}`}
    >
      {icon}
  ...
  return (
    <div className="card border-0 shadow-sm h-100">
      <div className="card-body d-flex align-items-center justify-content-between p-3">
        ...
        <div
          className={`text-${color} bg-light border border-${color} border-opacity-25 rounded d-flex align-items-center justify-content-center`}
...
const PlateCard = ({ item, formatTime, formatDate }) => {
  ...
  return (
    <div
      className={`card mb-3 shadow-sm border-0 border-start border-5 ${isSuccess ? "border-success" : "border-danger bg-white"}`}
    >
      <div className="card-body p-3">
        <div className="d-flex justify-content-between align-items-start mb-2">
          <div className="d-flex align-items-center gap-2">
            {isSuccess ? (
              <span className="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 text-uppercase">
                Hợp lệ
              </span>
            ) : (
          ...
          {isSuccess ? (
            ...
            <div>
              ...
              <div className="mt-2 p-2 bg-light border border-danger border-opacity-25 rounded">
                ...
                >
                  {item?.raw_5_reads?.join(" | ") ||
                    "Dữ liệu ảnh lỗi hoàn toàn"}
                </small>
              </div>
            </div>
          )}
