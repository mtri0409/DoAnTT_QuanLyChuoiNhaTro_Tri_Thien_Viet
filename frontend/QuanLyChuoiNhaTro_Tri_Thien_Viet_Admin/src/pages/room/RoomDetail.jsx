...
const RoomDetail = () => {
  ...
  // ========== HELPER: Icon cho tiện ích ==========
  const getAmenityIcon = (name) => {
    if (!name) return <FaStar size={14} />;
    const lower = name.toLowerCase();

    if (lower.includes('wifi') || lower.includes('internet') || lower.includes('mạng'))
    ...
    if (lower.includes('xe') || lower.includes('đỗ') || lower.includes('parking') || lower.includes('gara'))
      return <FaParking size={14} />;
    if (lower.includes('quạt'))
      return <FaFan size={14} />;
    if (lower.includes('điện'))
      return <FaBolt size={14} />;
    if (lower.includes('nước'))
      return <FaTint size={14} />;
    if (lower.includes('sofa') || lower.includes('ghế') || lower.includes('bàn') || lower.includes('nội thất'))
    ...
    if (lower.includes('khóa') || lower.includes('an ninh') || lower.includes('bảo vệ'))
      return <FaLock size={14} />;
    if (lower.includes('camera'))
      return <FaCamera size={14} />;

    return <FaCheck size={14} />;
  };

  // ========== HELPER: Màu cho tiện ích ==========
  const getAmenityColor = (index) => {
    const colors = [
      ...
      { bg: '#fff1f2', text: '#f43f5e', border: '#fecdd3' },  // pink
    ];
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case 'AVAILABLE':
        return { label: 'Có sẵn', bg: 'bg-success', icon: <FaCheck size={10} /> };
      case 'OCCUPIED':
        return { label: 'Đã cho thuê', bg: 'bg-warning', icon: <FaUsers size={10} /> };
      case 'MAINTENANCE':
        return { label: 'Bảo trì', bg: 'bg-danger', icon: <FaExclamationTriangle size={10} /> };
      default:
        return { label: status || 'Không rõ', bg: 'bg-secondary', icon: <FaInfoCircle size={10} /> };
    }
  };
            ...
            <div className="mt-3">
              <div className="d-flex justify-content-between small mb-1">
                ...
                <span className="fw-bold text-dark">
                  {room.maxPeople ? Math.round(((room.currentPeople || 0) / room.maxPeople) * 100) : 0}%
                </span>
              </div>
              <div className="progress" style={{ height: '8px', borderRadius: '4px' }}>
                <div
                  className={`progress-bar ${
                    ((room.currentPeople || 0) / (room.maxPeople || 1)) >= 1
                      ? 'bg-danger'
                      : ((room.currentPeople || 0) / (room.maxPeople || 1)) >= 0.7
                      ? 'bg-warning'
                      : 'bg-success'
                  }`}
                  style={{
            ...
            <div className="row g-3">
              ...
              <div className="col-md-4">
                <div className="bg-light rounded-3 p-3">
                  ...
                  <span className={`badge ${statusConfig.bg} px-2 py-1 rounded-pill`}>
                    {statusConfig.icon} {statusConfig.label}
                  </span>
                </div>
              </div>
                    ...
                    <div key={idx} className="col-6 col-md-4 col-xl-3">
                      ...
                      >
                        ...
                        <span 
                          ...
                          style={{ color: color.text, fontSize: '12px', lineHeight: '1.3' }}
                        >
                          {amenityName}
                        </span>
                      </div>
                ...
                {getAllMedia().map((media, idx) => (
                  <div key={idx} className="col-6 col-md-4">
                    ...
                    >
                      ...
                      {media.isThumbnail && (
                        <span className="badge bg-primary position-absolute top-0 start-0 m-2 rounded-pill" style={{ fontSize: '10px' }}>
                          Ảnh bìa
                        </span>
                      )}
              ...
              <div className="row g-3 small">
                <div className="col-md-6">
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    ...
                    <strong className="text-dark">#{room.roomId}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Tên phòng:</span>
                    <strong className="text-dark">{room.roomName || '—'}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Chi nhánh:</span>
                    <strong className="text-dark">{getBranchName(room.floorId)}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2">
                    <span className="text-muted">Tầng:</span>
                    <strong className="text-dark">{getFloorNumber(room.floorId)}</strong>
                  </div>
                </div>
                <div className="col-md-6">
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Giá thuê:</span>
                    <strong className="text-success">{formatPrice(room.price)}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Sức chứa:</span>
                    <strong className="text-dark">{room.currentPeople ?? 0}/{room.maxPeople ?? 1} người</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Tiện ích:</span>
                    <strong className="text-dark">{room.amenities?.length || 0} tiện ích</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2">
                    <span className="text-muted">Ảnh/Video:</span>
                    <strong className="text-dark">{getAllMedia().length} file</strong>
                  </div>
                </div>
              </div>
            </div>
