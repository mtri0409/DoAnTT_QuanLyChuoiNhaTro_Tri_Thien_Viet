...
import {
  ...
  FaArrowLeft, FaEdit, FaTrash, FaCheck, FaToggleOn,
  FaImage, FaMapMarkerAlt, FaLayerGroup, FaStar,
  FaChevronLeft, FaChevronRight, FaExpand, FaTimes,
  FaWifi, FaSnowflake, FaTv, FaShower, FaParking,
  FaInfoCircle, FaThLarge, FaExclamationTriangle,
  FaFan, FaBolt, FaTint, FaCouch, FaUtensils,
  FaFireExtinguisher, FaDoorOpen, FaLock, FaCamera,
  FaFileContract
} from 'react-icons/fa';
...
const RoomDetail = () => {
  ...
  const getAmenityIcon = (name) => {
    ...
    if (lower.includes('wifi') || lower.includes('internet') || lower.includes('mạng')) return <FaWifi size={14} />;
    if (lower.includes('điều hòa') || lower.includes('máy lạnh') || lower.includes('ac')) return <FaSnowflake size={14} />;
    if (lower.includes('tv') || lower.includes('tivi') || lower.includes('truyền hình')) return <FaTv size={14} />;
    if (lower.includes('nóng lạnh') || lower.includes('vòi sen') || lower.includes('tắm')) return <FaShower size={14} />;
    if (lower.includes('xe') || lower.includes('đỗ') || lower.includes('parking') || lower.includes('gara')) return <FaParking size={14} />;
    ...
    if (lower.includes('sofa') || lower.includes('ghế') || lower.includes('bàn') || lower.includes('nội thất')) return <FaCouch size={14} />;
    if (lower.includes('bếp') || lower.includes('nấu ăn')) return <FaUtensils size={14} />;
    if (lower.includes('pccc') || lower.includes('cháy')) return <FaFireExtinguisher size={14} />;
    if (lower.includes('ban công') || lower.includes('cửa')) return <FaDoorOpen size={14} />;
    if (lower.includes('khóa') || lower.includes('an ninh') || lower.includes('bảo vệ')) return <FaLock size={14} />;
  ...
  const handleDelete = async () => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa phòng này? Hành động này không thể hoàn tác.')) return;
    try {
  ...
  if (error || !room) return (
    <div className="container-fluid py-5">
      <div className="d-flex flex-column align-items-center justify-content-center" style={{ minHeight: '400px' }}>
        ...
        <p className="text-muted mb-4">{error || 'Phòng bạn tìm không tồn tại hoặc đã bị xóa.'}</p>
        <button onClick={() => navigate('/rooms/1')} className="btn btn-primary px-4 fw-bold">
    ...
    <div className="container-fluid py-4">
      ...
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h4 className="fw-bold text-dark mb-0 text-uppercase">Chi tiết phòng</h4>
            <p className="text-muted small mb-0">Mã phòng: <strong className="text-primary">#{room.roomId}</strong></p>
          </div>
        </div>
        <div className="d-flex gap-2">
          <button onClick={() => navigate(`/rooms/${room.roomId}/update`)}
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-5">
          ...
          <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
            {images.length > 0 ? (
              <>
                <div className="position-relative" style={{ height: '320px', background: '#f8f9fa' }}>
                  <img src={getFullImageUrl(images[currentImageIndex]?.url)} alt={`Phòng ${room.roomName}`}
                    className="w-100 h-100" style={{ objectFit: 'cover', cursor: 'pointer' }}
                  ...
                  <div className="position-absolute bottom-0 end-0 m-3">
                    <span className="badge bg-dark bg-opacity-75 px-3 py-2 rounded-pill">
                      <FaImage size={10} className="me-1" />{currentImageIndex + 1} / {images.length}
                    </span>
                  </div>
                    ...
                    <div className="d-flex gap-2 overflow-auto pb-1" style={{ scrollbarWidth: 'thin' }}>
                      {images.map((img, idx) => (
                        <img key={idx} src={getFullImageUrl(img.url)} alt={`Thumb ${idx + 1}`}
                          ...
                          style={{ width: '64px', height: '48px', objectFit: 'cover', cursor: 'pointer' }}
              ...
              <div className="d-flex flex-column align-items-center justify-content-center bg-light" style={{ height: '320px' }}>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4">
            ...
            <div className="d-flex align-items-start justify-content-between mb-4">
              <div>
                <h3 className="fw-bold text-dark mb-1">{room.roomName || '—'}</h3>
                <div className="d-flex align-items-center gap-2 text-muted small">
                  <FaMapMarkerAlt size={12} /><span>{getBranchName(room.floorId)}</span>
                  <span>•</span>
                  <FaLayerGroup size={12} /><span>{getFloorNumber(room.floorId)}</span>
                </div>
              </div>
              <span className={`badge ${statusConfig.bg} px-3 py-2 rounded-pill d-inline-flex align-items-center gap-1`}>
                {statusConfig.icon} {statusConfig.label}
              </span>
            </div>
            <div className="bg-success bg-opacity-10 rounded-3 p-3 mb-4 text-center">
              <small className="text-muted fw-bold d-block mb-1">GIÁ THUÊ</small>
              <h4 className="fw-bold text-success mb-0"><FaDollarSign size={18} className="me-1" />{formatPrice(room.price)}</h4>
            ...
            <div className="row g-3">
              <div className="col-6">
                <div className="bg-light rounded-3 p-3 text-center">
                  ...
                  <small className="text-muted fw-bold d-block">NGƯỜI HIỆN TẠI</small>
                  <h5 className="fw-bold text-dark mb-0 mt-1">{room.currentPeople ?? 0}</h5>
                </div>
              </div>
              <div className="col-6">
                <div className="bg-light rounded-3 p-3 text-center">
                  <FaUsers className="text-info mb-2" size={20} />
                  <small className="text-muted fw-bold d-block">SỨC CHỨA TỐI ĐA</small>
                  <h5 className="fw-bold text-dark mb-0 mt-1">{room.maxPeople ?? 1}</h5>
                </div>
              </div>
            </div>
            <div className="mt-3">
        ...
        <div className="col-lg-7">
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
            ...
            <p className="text-dark mb-0 lh-lg" style={{ whiteSpace: 'pre-line' }}>
              {room.description || <span className="text-muted fst-italic">Chưa có mô tả cho phòng này.</span>}
            </p>
          </div>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
            <div className="d-flex align-items-center gap-2 mb-4 border-bottom pb-3">
              <div className="bg-warning-subtle p-2 rounded-3 text-warning"><FaBuilding size={20} /></div>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
            ...
            {room.depositAmount ? (
              <div className="row g-3">
                <div className="col-6">
                  <div className="bg-warning bg-opacity-10 rounded-3 p-3 text-center">
                    <small className="text-muted fw-bold d-block mb-1">SỐ TIỀN CỌC</small>
                    <h5 className="fw-bold text-warning mb-0">{formatPrice(room.depositAmount)}</h5>
                  </div>
                </div>
                <div className="col-6">
                  <div className="bg-light rounded-3 p-3 text-center">
              ...
              <div className="bg-light rounded-3 p-4 text-center">
                <FaDollarSign size={24} className="text-muted opacity-25 mb-2" />
                <p className="text-muted small mb-0">Phòng này chưa có tiền cọc</p>
              </div>
            )}
          </div>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
            ...
            {loadingMembers ? (
              ...
              <div className="bg-light rounded-3 p-4 text-center">
                ...
                <p className="text-muted" style={{ fontSize: 12 }}>Khi phòng được cho thuê, hợp đồng và danh sách thành viên sẽ hiển thị ở đây</p>
              </div>
            ) : (
              <>
                {/* Contract info bar */}
                ...
                {members.length > 0 ? (
                      ...
                      return (
                        <div key={member.memberId ?? member.profileId ?? idx}
                          ...
                          <div className="rounded-circle bg-primary d-flex align-items-center justify-content-center flex-shrink-0"
                            ...
                            <span className="text-white fw-bold small">{initial}</span>
                          </div>
                          <div className="flex-grow-1 min-w-0">
                            <div className="fw-semibold text-dark small">{fullName}</div>
                            <div className="text-muted" style={{ fontSize: 12 }}>
                              {phone && <span className="me-2">📱 {phone}</span>}
                  ...
                  <div className="bg-light rounded-3 p-4 text-center">
                    ...
                    <p className="text-muted small mb-0">Hợp đồng chưa có thành viên nào</p>
                  </div>
                )}
              </>
            )}
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
            <div className="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
              <div className="d-flex align-items-center gap-2">
                <div className="bg-success-subtle p-2 rounded-3 text-success"><FaStar size={20} /></div>
                <h6 className="fw-bold mb-0 text-success">Tiện ích phòng</h6>
              </div>
              {room.amenities?.length > 0 && (
                <span className="badge bg-success-subtle text-success rounded-pill px-3">{room.amenities.length} tiện ích</span>
              )}
            </div>
            {room.amenities?.length > 0 ? (
              <div className="row g-2">
                {room.amenities.map((amenity, idx) => {
                  ...
                  const amenityName = amenity.amenityName || amenity.name || `Tiện ích #${amenity.amenityId}`;
                  return (
              ...
              <div className="bg-light rounded-3 p-4 text-center">
                ...
                <p className="text-muted small mb-0">Phòng này chưa có tiện ích nào</p>
              </div>
            )}
          </div>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-4">
            <div className="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
              <div className="d-flex align-items-center gap-2">
                <div className="p-2 rounded-3" style={{ backgroundColor: '#f3e8ff' }}>
                  <FaThLarge size={20} style={{ color: '#8b5cf6' }} />
                </div>
                <h6 className="fw-bold mb-0" style={{ color: '#8b5cf6' }}>Thư viện ảnh / video</h6>
              </div>
              {getAllMedia().length > 0 && (
                <span className="badge rounded-pill px-3" style={{ backgroundColor: '#f3e8ff', color: '#8b5cf6' }}>
            ...
            {getAllMedia().length > 0 ? (
              <div className="row g-3">
                {getAllMedia().map((media, idx) => (
                  <div key={idx} className="col-6 col-md-4">
                    <div className="position-relative rounded-3 overflow-hidden border" style={{ height: '140px', cursor: 'pointer' }}
                      onClick={() => {
                        const imgIdx = images.findIndex(img => img.url === media.url);
                        if (imgIdx !== -1) { setCurrentImageIndex(imgIdx); setShowLightbox(true); }
                      }}>
                      {media.mediaType?.startsWith('image') ? (
                        <img src={getFullImageUrl(media.url)} alt={`Media ${idx + 1}`} className="w-100 h-100" style={{ objectFit: 'cover' }} />
                      ) : (
                      ...
                      {media.isThumbnail && (
                        <span className="badge bg-primary position-absolute top-0 start-0 m-2 rounded-pill" style={{ fontSize: '10px' }}>Ảnh bìa</span>
                      )}
                      <span className="badge bg-dark bg-opacity-75 position-absolute bottom-0 end-0 m-2 rounded-pill" style={{ fontSize: '10px' }}>
                        {media.mediaType?.split('/')[1]?.toUpperCase() || 'FILE'}
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div className="bg-light rounded-3 p-4 text-center">
                ...
                <p className="text-muted small mb-0">Chưa có ảnh hoặc video nào</p>
              </div>
            )}
          </div>
              ...
              <div className="row g-3 small">
                <div className="col-md-6">
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Mã phòng:</span><strong>#{room.roomId}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Tên phòng:</span><strong>{room.roomName || '—'}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Chi nhánh:</span><strong>{getBranchName(room.floorId)}</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2">
                    <span className="text-muted">Tầng:</span><strong>{getFloorNumber(room.floorId)}</strong>
                  </div>
                </div>
                <div className="col-md-6">
                  <div className="d-flex justify-content-between py-2 border-bottom">
                  ...
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Sức chứa:</span><strong>{room.currentPeople ?? 0}/{room.maxPeople ?? 1} người</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2 border-bottom">
                    <span className="text-muted">Tiện ích:</span><strong>{room.amenities?.length || 0} tiện ích</strong>
                  </div>
                  <div className="d-flex justify-content-between py-2">
                    <span className="text-muted">Ảnh/Video:</span><strong>{getAllMedia().length} file</strong>
                  </div>
                </div>
              </div>
            </div>
      ...
      {showLightbox && images.length > 0 && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center"
          ...
          <img src={getFullImageUrl(images[currentImageIndex]?.url)} alt="Fullscreen"
