...
const AmenityList = () => {
    ...
    const handleAddAmenity = async () => {
        ...
        try {
            ...
            console.log('✅ Amenity created:', res);
            ...
            alert('Lỗi khi thêm tiện ích: ' + err.message);
        } finally {
    ...
    const handleDeleteAmenity = async (amenityId, amenityName) => {
        if (!window.confirm(`Bạn có chắc muốn xóa tiện ích "${amenityName}"?`)) {
            return;
        }
        ...
        <div className="container-fluid py-4">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p className="text-muted small mb-0">Hệ thống quản lý tiện ích phòng</p>
                </div>
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 rounded-4">
                            ...
                            <div className="modal-body">
                                <div className="mb-3">
                                    <label className="form-label small fw-bold text-muted">TÊN TIỆN ÍCH <span className="text-danger">*</span></label>
                                    <input
                                        type="text"
                                        className="form-control bg-light border-0 py-2"
                                        placeholder="VD: WiFi, TV, Điều hòa..."
                                        value={newAmenityData.amenityName}
                                        onChange={(e) => setNewAmenityData({ ...newAmenityData, amenityName: e.target.value })}
                                    />
                                </div>

                                <div className="mb-3">
                                    <label className="form-label small fw-bold text-muted">ICON (tuỳ chọn)</label>
                                    <input
                                        type="text"
                                        className="form-control bg-light border-0 py-2"
                                        placeholder="VD: wifi, tv, wind..."
                                        value={newAmenityData.icon}
                                        onChange={(e) => setNewAmenityData({ ...newAmenityData, icon: e.target.value })}
                                    />
                                </div>
                            </div>
            ...
            <div className="card border-0 shadow-sm rounded-3">
                            ...
                            {loading ? (
                                ...
                                data.content.map((item) => (
                                    <tr key={item.amenityId}>
                                        ...
                                        <td>
                                            <span className="text-muted small">{item.icon || '-'}</span>
                                        </td>
                                        <td className="text-end pe-4">
                                            <div className="d-flex justify-content-end gap-1">
                                                ...
                                                <button
                                                    className="btn btn-sm btn-light border-0"
                                                    onClick={() => handleDeleteAmenity(item.amenityId, item.amenityName)}
                ...
                <div className="card-footer bg-white py-3 d-flex justify-content-between align-items-center border-0 flex-wrap gap-2">
                    <small className="text-muted">
                        Tổng: <strong>{data?.totalElements || 0}</strong> tiện ích | Trang: <strong>{(currentPage + 1)}/{data?.totalPages || 1}</strong>
                    </small>
                    {data?.totalPages > 1 && (
                        <Pagination
